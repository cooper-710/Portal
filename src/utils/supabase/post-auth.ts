...
/**
 * After magic-link / OTP / password sign-in:
 * 1. Users without a password → onboarding password setup
 * 2. Freelancers without workspace access (and a generic /dashboard next)
 *    → /dashboard/billing so they can start/renew
 * 3. Freelancers with workspace access (trialing/active) whose next is billing
 *    → /dashboard (working overview), not billing
 * 4. Freelancers with access who have not customized/skipped portal branding
 *    → /onboarding/portal
 * Clients always keep dashboard (or their requested next).
 */
export async function resolvePostAuthPath(nextPath = "/dashboard") {
